...
def get_dirichlet_data_loaders(dataset, num_clients, alpha, batch_size, min_samples):
    ...
    while True:
        ...
        for c in range(num_classes):
            ...
            for i in range(num_clients):
                client_indices[i].extend(class_indices[c][cur: cur + num_samples_per_client[i]])
                cur += num_samples_per_client[i]
    ...
    for i in range(num_clients):
        ...
        train_loaders.append(DataLoader(Subset(dataset, client_indices[i][:train_len]), batch_size=batch_size, shuffle=True))
        test_loaders.append(DataLoader(Subset(dataset, client_indices[i][train_len:]), batch_size=batch_size, shuffle=False))
...
def main():
    ...
    for rnd in range(1, GLOBAL_ROUNDS + 1):
        ...
        for idx in range(NUM_CLIENTS):
            w, loss, size = local_train(global_model, client_train_loaders[idx], LOCAL_EPOCHS, DEVICE)
    ...
    plt.title('PathMNIST FedAvg: Communication Rounds vs Avg Client Accuracy')
    plt.xlabel('Communication Rounds')
    plt.ylabel('Accuracy (%)')
    plt.grid(True)
    plt.legend()
    plt.savefig('PathMNIST_fedavg_client_accuracy.png')
    plt.close()

    plt.figure(figsize=(10, 5))
    plt.plot(rounds, history['global_acc'], marker='s', color='red', label='Global Model Acc')
    plt.title('PathMNIST FedAvg: Communication Rounds vs Global Accuracy')
    plt.xlabel('Communication Rounds')
    plt.ylabel('Accuracy (%)')
    plt.grid(True)
    plt.legend()
    plt.savefig('PathMNIST_fedavg_global_accuracy.png')
    plt.close()

    plt.figure(figsize=(10, 6))
    plt.plot(rounds, history['avg_client_acc'], marker='o', color='blue', label='Avg Client Acc')
    plt.plot(rounds, history['global_acc'], marker='s', color='red', label='Global Model Acc')
    plt.title('PathMNIST FedAvg: Communication Rounds vs Federated Accuracies')
    plt.xlabel('Communication Rounds')
    plt.ylabel('Accuracy (%)')
    plt.grid(True)
    plt.legend()
    plt.savefig('PathMNIST_fedavg_combined.png')
    plt.close()

    print("\n[Done] Plots saved to project folder.")
